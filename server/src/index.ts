...
export function makeMcpServer(
  ...
  // --- Free tool: get_priority_fees ---
  server.registerTool(
    "get_priority_fees",
    {
      description:
        "Get current Solana priority fee estimates. " +
        "Free — no payment required. Returns fee levels in micro-lamports " +
        "per compute unit. Requires a Helius RPC endpoint.",
      inputSchema: {
        account_keys: z
          .array(z.string())
          .optional()
          .describe(
            "Optional list of Solana account keys to scope the fee estimate. " +
            "Providing relevant accounts improves accuracy."
          ),
      },
      outputSchema: {
        priority_fee_levels: z.object({
          min: z.number().nullable(),
          low: z.number().nullable(),
          medium: z.number().nullable(),
          high: z.number().nullable(),
          very_high: z.number().nullable(),
        }),
    ...
    async ({ account_keys }) => {
      // Per-IP rate limiting (reuse free tool limits)
      if (clientIp) {
        const now = Date.now();
        const usage = freeToolIpLimits.get(clientIp);
        if (usage && now < usage.resetAt) {
      ...
      try {
        ...
        return {
          ...
          structuredContent: feeData,
        };
      } catch (error: unknown) {
        const message =
          error instanceof Error ? error.message : String(error);
        // If the RPC doesn't support getPriorityFeeEstimate (not Helius), return a helpful error
        if (
          ...
          return {
            content: [
              {
                type: "text",
                text: "Priority fee estimation requires a Helius RPC endpoint. " +
                  "Set SOLANA_RPC_URL to your Helius RPC URL (e.g., https://mainnet.helius-rpc.com/?api-key=YOUR_KEY).",
              },
            ],
            isError: true,
          };
        ...
        return {
          content: [
            {
              type: "text",
              text: `Failed to estimate priority fees: ${message}`,
            },
          ],
          isError: true,
        };
